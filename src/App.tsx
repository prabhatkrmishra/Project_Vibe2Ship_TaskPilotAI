        ...
        <div className="flex flex-col h-full bg-[#0d1117]">
            ...
            <nav className="flex-1 px-4 space-y-2">
                <Link to="/dashboard" onClick={onClose}
                      className={`flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-900 transition-colors text-slate-300 ${location.pathname === '/dashboard' ? 'bg-indigo-500/10 border-l-2 border-indigo-500 pl-[10px]' : 'border-l-2 border-transparent pl-[10px]'}`}>
                    <LayoutDashboard className="h-4 w-4 text-indigo-400"/> Command Center
                </Link>
                <Link to="/timetable" onClick={onClose}
                      className={`flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-900 transition-colors text-slate-300 ${location.pathname === '/timetable' ? 'bg-indigo-500/10 border-l-2 border-indigo-500 pl-[10px]' : 'border-l-2 border-transparent pl-[10px]'}`}>
                    <Calendar className="h-4 w-4 text-pink-400"/> Timetable
                </Link>
                <Link to="/tasks" onClick={onClose}
                      className={`flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-900 transition-colors text-slate-300 ${location.pathname === '/tasks' ? 'bg-indigo-500/10 border-l-2 border-indigo-500 pl-[10px]' : 'border-l-2 border-transparent pl-[10px]'}`}>
                    <CheckSquare className="h-4 w-4 text-emerald-400"/> Mission Board
                </Link>
                <Link to="/goals" onClick={onClose}
                      className={`flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-900 transition-colors text-slate-300 ${location.pathname === '/goals' ? 'bg-indigo-500/10 border-l-2 border-indigo-500 pl-[10px]' : 'border-l-2 border-transparent pl-[10px]'}`}>
                    <Target className="h-4 w-4 text-cyan-400"/> Quest & Habit
                </Link>
                <Link to="/focus" onClick={onClose}
                      className={`flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-900 transition-colors text-slate-300 ${location.pathname === '/focus' ? 'bg-indigo-500/10 border-l-2 border-indigo-500 pl-[10px]' : 'border-l-2 border-transparent pl-[10px]'}`}>
                    <Headphones className="h-4 w-4 text-violet-400"/> Focus Zone
                </Link>
                <Link to="/completions" onClick={onClose}
                      className={`flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-900 transition-colors text-slate-300 ${location.pathname === '/completions' ? 'bg-indigo-500/10 border-l-2 border-indigo-500 pl-[10px]' : 'border-l-2 border-transparent pl-[10px]'}`}>
                    <CheckCircle2 className="h-4 w-4 text-emerald-400"/> Completions
                </Link>
                <Link to="/workspace" onClick={onClose}
                      className={`flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-900 transition-colors text-slate-300 ${location.pathname === '/workspace' ? 'bg-indigo-500/10 border-l-2 border-indigo-500 pl-[10px]' : 'border-l-2 border-transparent pl-[10px]'}`}>
                    <Cloud className="h-4 w-4 text-amber-400"/> Workspace Actions
                </Link>
                <Link to="/chat" onClick={onClose}
                      className={`flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-900 transition-colors text-slate-300 ${location.pathname === '/chat' ? 'bg-indigo-500/10 border-l-2 border-indigo-500 pl-[10px]' : 'border-l-2 border-transparent pl-[10px]'}`}>
                    <MessageSquare className="h-4 w-4 text-violet-400"/> Mission Control
                </Link>
                {user?.role === 'admin' && (
                    <Link to="/admin" onClick={onClose}
                          className={`flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-900 transition-colors text-slate-300 ${location.pathname === '/admin' ? 'bg-indigo-500/10 border-l-2 border-indigo-500 pl-[10px]' : 'border-l-2 border-transparent pl-[10px]'}`}>
            ...
            <div className="p-4 border-t border-[#21262d]">
                ...
                >
                    {user?.picture ? (
                        ...
                        <div
                            className="w-12 h-12 rounded-xl bg-slate-800 flex items-center justify-center text-slate-200 font-bold ring-2 ring-indigo-500/30 shrink-0 text-base shadow-md">
                            {user?.name?.charAt(0) || user?.email?.charAt(0) || 'U'}
                        </div>
                    )}
                    <div className="flex-1 min-w-0 text-left">
                        <p className="text-sm font-semibold text-[#f0f6fc] truncate leading-tight">
                            {user?.name || 'User'}
                        </p>
                        ...
                        <div className="mt-2">
                            {user?.isPremium ? (
                                <span
                                    className="text-[10px] font-bold text-violet-400 bg-violet-500/15 px-2 py-0.5 rounded-lg border border-violet-500/30">
                                    Premium
                                </span>
                            ) : (
                                <span
                ...
                <div className="flex justify-center gap-3 text-[11px] text-slate-500 mb-4">
                    <Link to="/privacy" onClick={onClose} className="hover:text-indigo-400 transition-colors">Privacy
                        Policy</Link>
                    <span>•</span>
                    <Link to="/terms" onClick={onClose} className="hover:text-indigo-400 transition-colors">Terms of
                        Service</Link>
                </div>
                <Button variant="outline"
                        className="w-full justify-start text-slate-400 border-slate-800 bg-slate-900 hover:bg-slate-800 hover:text-white rounded-xl cursor-pointer"
                        onClick={() => {
                            logout();
                            if (onClose) onClose();
                        }}>
...
function Layout({children}: { children: React.ReactNode }) {
    ...
    return (
        <div className="flex h-screen bg-[#030712] text-slate-200 font-sans overflow-hidden flex-col lg:flex-row">
            {/* Mobile Header */}
            <div
                className="lg:hidden flex items-center justify-between px-6 py-4 bg-[#0d1117] border-b border-[#21262d] shrink-0">
                ...
                <div className="flex items-center gap-3">
                    ...
                    >
                        {user?.picture ? (
                            ...
                            <div
                                className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center text-slate-200 font-semibold ring-2 ring-indigo-500/10 shrink-0 text-xs">
                                {user?.name?.charAt(0) || user?.email?.charAt(0) || 'U'}
                            </div>
                        )}
                    </Link>
            ...
            <aside
                className="w-64 xl:w-72 2xl:w-80 border-r border-[#21262d] bg-[#0d1117] flex-col shrink-0 hidden lg:flex h-full transition-all duration-300">
        ...
        <div className="login-bg flex min-h-screen items-center justify-center text-slate-200 py-10 px-4">
            <div className="w-full max-w-md p-8 bg-[#0d1117] border border-[#21262d] rounded-3xl shadow-2xl space-y-6">
                <div className="text-center space-y-2">
                    <div
                        className="mx-auto w-14 h-14 bg-indigo-600 rounded-2xl flex items-center justify-center shadow-[0_0_30px_rgba(99,102,241,0.3)] animate-in zoom-in duration-500">
                ...
                {!isRegister && (
                    <div className="flex flex-wrap gap-1.5 justify-center py-1">
                        <span
                            className="px-2 py-0.5 rounded-full bg-[#161b22] border border-[#21262d] text-[10px] text-indigo-300 font-medium font-mono">⚡ AI Tasks</span>
                        <span
                            className="px-2 py-0.5 rounded-full bg-[#161b22] border border-[#21262d] text-[10px] text-cyan-300 font-medium font-mono">📅 Workspace Sync</span>
                        <span
                            className="px-2 py-0.5 rounded-full bg-[#161b22] border border-[#21262d] text-[10px] text-emerald-300 font-medium font-mono">🎯 Risk Scoring</span>
                    </div>
                )}
                ...
                {twoFARequired ? (
                    <form onSubmit={handleVerify2FA} className="space-y-4">
                        <div className="text-center space-y-2 pb-2">
                            <div
                                className="mx-auto w-10 h-10 bg-amber-500/20 rounded-xl flex items-center justify-center">
                                <svg className="h-5 w-5 text-amber-400" fill="none" viewBox="0 0 24 24" strokeWidth={2}
                                     stroke="currentColor">
                        ...
                        {error && (
                            <div
                                className="text-xs text-rose-400 bg-rose-500/10 border border-rose-500/20 px-4 py-2.5 rounded-xl text-center font-medium animate-in fade-in slide-in-from-top-1 duration-200">
                                {error}
                            </div>
                        )}
