...
export default function FocusTimer({
  ...
  const config = useMemo(() => {
    if (!method) return { label: 'Unknown', mode: 'countdown' as TimerMode, workSeconds: 0, breakSeconds: 0, cycles: 0 };
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      {/* Break suggestions */}
      {phase === 'break' && (
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-4 py-2 text-center max-w-xs">
      ...
      <div className="text-[10px] text-slate-600 text-center">
        Space = {running ? 'pause' : 'start'} &middot; R = reset{phase === 'break' ? ' &middot; S = skip break' : ''}
      </div>
    </div>
  );
}
