...
export function ActiveSessionCard({ plan }: ActiveSessionCardProps) {
  ...
  if (!plan) {
    return (
      <div className={`${PANEL} flex items-center gap-3`}>
        <span className="px-2 py-1 bg-[#161b22] text-slate-500 text-[10px] font-bold rounded uppercase tracking-wider border border-[#21262d]">
          Active Session
        </span>
  ...
  if (!activeSession) {
    return (
      <div className={`${PANEL} flex items-center gap-3`}>
        <span className="px-2 py-1 bg-[#161b22] text-slate-500 text-[10px] font-bold rounded uppercase tracking-wider border border-[#21262d] shrink-0">
          Active Session
        </span>
        <div className="min-w-0 flex items-start gap-2">
          ...
          <p className="text-xs text-slate-400 break-words">
            {nextSession
              ? <>No session in progress &middot; Next: <span className="text-slate-300 font-medium">{nextSession.sessionLabel || nextSession.taskTitle}</span> at {formatClock(nextSession.startTime)}</>
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute -top-10 -right-10 w-32 h-32 rounded-full bg-gradient-to-br from-cyan-400/30 to-cyan-500/0 blur-2xl opacity-70" />

      <div className="relative flex items-center justify-between mb-4">
        <span className="px-2 py-1 bg-cyan-500/20 text-cyan-400 text-[10px] font-bold rounded uppercase tracking-wider flex items-center gap-1.5">
      ...
      <div className="relative flex items-start gap-4">
        <CircularProgress progress={progress} size={48} strokeWidth={4} color="stroke-cyan-400" trackColor="stroke-[#21262d]">
