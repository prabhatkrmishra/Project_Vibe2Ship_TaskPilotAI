    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-6 justify-center">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-sm text-slate-300">Streak: <span className="font-bold text-orange-300">{stats.focusStreak}d</span></span>
        </div>
        <div className="flex items-center gap-1.5">
          <Target className="h-4 w-4 text-purple-400" />
          <span className="text-sm text-slate-300">Best: <span className="font-bold text-purple-300">{stats.longestFocusStreak}d</span></span>
        </div>
        <div className="text-sm text-slate-400">
          Total: <span className="font-bold text-slate-200">{stats.totalFocusMinutes}m</span> ({stats.totalFocusSessions} sessions)
        </div>
      </div>
...
function StatCard({ icon, label, value, sub }: { icon: React.ReactNode; label: string; value: string; sub: string }) {
  return (
    <div className="bg-slate-800/40 border border-slate-700/30 rounded-xl p-3 text-center">
      <div className="flex items-center justify-center gap-1.5 mb-1">{icon}<span className="text-[10px] uppercase tracking-wider text-slate-500">{label}</span></div>
