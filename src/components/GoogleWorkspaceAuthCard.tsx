...
export function GoogleWorkspaceAuthCard({
  ...
  if (status === 'checking') {
    return (
      <div className="bg-[#0d1117] border border-[#21262d] rounded-3xl p-5 flex items-center gap-3 text-slate-400">
  ...
  if (status === 'connected') {
    return (
      <div className="bg-[#0d1117] border border-emerald-500/30 rounded-3xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="w-11 h-11 rounded-2xl border border-emerald-400/40 bg-emerald-500/15 flex items-center justify-center text-emerald-300 shrink-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-slate-400 mt-0.5 truncate">
              {googleEmail ? (
                <>Authorized as <span className="text-slate-300 font-medium">{googleEmail}</span> — Calendar, Drive, Docs, Sheets, Slides &amp; Tasks are unlocked below.</>
              ) : (
  ...
  return (
    <div className="bg-[#0d1117] border border-amber-500/30 rounded-3xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div className="flex items-center gap-4">
        <div className="w-11 h-11 rounded-2xl border border-amber-400/40 bg-amber-500/15 flex items-center justify-center text-amber-300 shrink-0">
          <ShieldAlert className="h-5 w-5" />
        </div>
        <div className="min-w-0">
          <h3 className="text-sm font-bold text-[#f0f6fc]">Google Account Not Authorized</h3>
          <p className="text-xs text-slate-400 mt-0.5">
            Signing in to TaskPilot doesn't grant Workspace access. Connect a Google account to unlock Calendar, Drive, Docs, Sheets, Slides &amp; Tasks actions below.
          </p>
        </div>
      </div>
      <Button
