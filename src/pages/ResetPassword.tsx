...
export default function ResetPassword() {
  ...
  if (!token || tokenValid === false) {
    return (
      <div className="login-bg flex min-h-screen items-center justify-center text-slate-200 py-10 px-4">
        <div className="w-full max-w-md p-8 bg-[#0d1117] border border-[#21262d] rounded-3xl shadow-2xl space-y-6 text-center">
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setErrorMsg(null);
    if (!newPassword || !retypePassword) { setErrorMsg("Please fill in all fields."); return; }
    if (newPassword.length < 8) { setErrorMsg("Password must be at least 8 characters."); return; }
    if (newPassword !== retypePassword) { setErrorMsg("Passwords do not match."); return; }
    try {
      setSubmitting(true);
        ...
        {result === 'success' ? (
          <div className="space-y-4">
            <div className="flex items-center gap-3 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
              ...
              <p className="text-sm text-emerald-300">Your password has been changed. You can now sign in with your new password.</p>
            </div>
            <Link
              to="/login"
              className="block w-full h-11 text-center text-xs uppercase tracking-widest font-bold bg-indigo-600 text-white rounded-xl hover:bg-indigo-500 transition-colors leading-[44px]"
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1 text-left">
              <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider font-mono">New Password</label>
            ...
            <div className="space-y-1 text-left">
              ...
              <div className="relative">
                <input
                  ...
                  onChange={(e) => { setRetypePassword(e.target.value); setErrorMsg(null); }}
                  placeholder="••••••••"
                  required
                  className="w-full pl-10 pr-4 py-2 bg-slate-900 border border-[#21262d] rounded-xl text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500 text-sm transition-all"
                />
