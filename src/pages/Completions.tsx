...
export function Completions() {
    ...
    return (
        <div
            className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto space-y-6 select-none bg-[var(--graphite-950)] w-full">
            <PageHeader
                icon={Award}
                badge="Completions"
                color="emerald"
            ...
            {loading ? (
                <div className="text-center text-slate-400 py-12">Loading completions board...</div>
            ) : (
                <div className="space-y-6">
                    {/* Stats Bar */}
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div className="p-4 bg-[var(--graphite-950)] border border-[var(--panel-line)] rounded-2xl flex items-center gap-3">
                            <div
                                className="w-10 h-10 rounded-xl bg-cyan-500/10 flex items-center justify-center text-cyan-400">
                                <Sparkles className="w-5 h-5"/>
                            </div>
                            <div>
                                <p className="text-[10px] text-slate-400 uppercase tracking-widest font-mono">Quests
                                    Completed</p>
                        ...
                        <div className="p-4 bg-[var(--graphite-950)] border border-[var(--panel-line)] rounded-2xl flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-slate-400 uppercase tracking-widest font-mono">Tasks
                                    Completed</p>
                            </div>
                        </div>
                        <div className="p-4 bg-[var(--graphite-950)] border border-[var(--panel-line)] rounded-2xl flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-[10px] text-slate-400 uppercase tracking-widest font-mono">On-Time
                                    Rate</p>
                                <p className="text-xl font-bold text-slate-100">
                                    {(() => {
                    ...
                    {!hasCompletions ? (
                        <div
                            className="text-center py-24 bg-[var(--graphite-900)] rounded-3xl border border-dashed border-[var(--panel-line)]">
                            <CheckCircle2 className="mx-auto h-12 w-12 text-slate-500 mb-4"/>
                            <h3 className="text-lg font-medium text-white">No completions logged yet</h3>
                            <p className="text-slate-400 max-w-sm mx-auto mt-1 text-xs">Completed quests and scheduled
                                tasks from your Mission Board will be celebrated right here.</p>
                        </div>
                    ) : (
                        <div className="space-y-6">
                                        ...
                                        {completedQuests.map(quest => {
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    className="bg-[var(--graphite-900)] border-2 border-emerald-500/20 rounded-3xl p-5 relative overflow-hidden flex flex-col gap-3 cursor-pointer hover:border-emerald-500/40 transition-colors"
                                                >
                                                    <div
                                                        className="absolute top-0 right-0 w-20 h-20 bg-emerald-500/5 rounded-full blur-xl pointer-events-none"></div>

                                                    <div className="flex justify-between items-center">
                                                        <div className="flex items-center gap-2">
                                                            <span
                                                                className="inline-block w-2 h-2 rounded-full bg-emerald-400 shadow-md shadow-emerald-400/50"></span>
                                                            <span
                                                                className="text-xs uppercase tracking-widest font-mono font-bold text-emerald-400">Complete</span>
                                                        </div>
                                                        <span
                                                            className="px-2 py-0.5 text-[9px] font-mono font-bold bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 rounded uppercase tracking-wider">
                              Quest
                            </span>
                                                    </div>
                                                    ...
                                                    <div className="flex justify-between items-start mt-1">
                                                        ...
                                                        {delay && (
                                                            <span
                                                                className={`px-2 py-0.5 text-[9px] font-mono font-bold rounded uppercase tracking-wider border ${delay.color}`}>
                                {delay.text}
                              </span>
                                                        )}
                                                    </div>
                                                    ...
                                                    {isExpanded ? (
                                                        <motion.div
                                                            ...
                                                            className="space-y-3 pt-3 border-t border-[var(--panel-line)] mt-2"
                                                        >
                                                            {quest.description && (
                                                            ...
                                                            <div className="pt-1">
                                                                <p className="text-[9px] uppercase tracking-wider font-bold text-slate-500 mb-2">Quest
                                                                    Tasks:</p>
                                                                <div className="space-y-2">
                                                                    {questTasks.map(t => {
                                                                        ...
                                                                        return (
                                                                            <div key={t.id}
                                                                                 className="flex items-center justify-between gap-2 p-2 bg-[var(--graphite-900)] border border-[var(--panel-line)]/50 rounded-xl text-xs">
                                                                                <span
                                                                                    className="text-slate-300 font-medium truncate flex-1 leading-snug">{t.title}</span>
                                                                                {tDelay && (
                                                                                    <span
                                                                                        className={`px-1.5 py-0.5 text-[8px] font-mono font-semibold rounded shrink-0 border ${tDelay.color}`}>
                                            {tDelay.text}
                                          </span>
                                                                                )}
                                                                            </div>
                                                        ...
                                                        <div
                                                            className="text-[10px] text-slate-500 italic text-center mt-1 border-t border-[var(--panel-line)]/40 pt-2 font-light">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {activeQuestsWithCompletions.map(quest => {
                                            ...
                                            return (
                                                <div key={quest.id}
                                                     className="bg-[var(--graphite-900)] border border-[var(--panel-line)] rounded-3xl p-5 flex flex-col gap-3">
                                                    <div>
                            <span
                                className="px-1.5 py-0.5 bg-amber-500/10 text-amber-400 border border-amber-500/20 text-[8px] font-bold uppercase tracking-wider rounded">
                              Quest Active
                            </span>
                                      ...
                                      {tDelay.text}
                                                    ...
                                                    {questTrails[quest.id] && questTrails[quest.id].length > 0 && (
                                                        <div className="pt-2 border-t border-[var(--panel-line)]/50">
                                                            <p className="text-[9px] uppercase tracking-wider font-bold text-slate-500 mb-2 flex items-center gap-1.5">
                                                                <Route className="w-3 h-3"/> Session Trail
                                                            </p>
                                                            <div className="space-y-0 relative">
                            ...
                            {standaloneCompletedTasks.length > 0 && (
                                <div className="space-y-3">
                                    <h4 className="text-xs uppercase tracking-widest font-bold text-emerald-400 pl-1 flex items-center gap-2">
                                        <CheckCircle2 className="w-3.5 h-3.5"/> Standalone Completed Tasks
                                        ({standaloneCompletedTasks.length})
                                    </h4>
                                    <div className="bg-[var(--graphite-900)] border border-[var(--panel-line)] rounded-3xl p-5">
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            {standaloneCompletedTasks.map(t => {
                                                const tDelay = getDelayText(t.deadline, t.completedAt);
                                                return (
                                                    <div key={t.id}
                                                         className="flex items-center justify-between gap-3 p-3 bg-[var(--graphite-900)] border border-[var(--panel-line)] rounded-2xl text-xs">
                                                        <span
                                                            className="text-slate-200 font-semibold truncate flex-1 leading-snug">{t.title}</span>
                                                        {tDelay && (
                                                            <span
                                                                className={`px-1.5 py-0.5 text-[8px] font-mono font-semibold rounded shrink-0 border ${tDelay.color}`}>
                                {tDelay.text}
