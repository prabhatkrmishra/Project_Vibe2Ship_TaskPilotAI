            ...
            >
                    ...
                    <div className="flex gap-6">
                        ...
                        {decisions.length > 0 && (
                            ...
                            >
                                ...
                                Latest AI Insight
                                <span
                                    className="text-[9px] bg-emerald-500/15 text-emerald-400 px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wider">Live</span>
                            </button>
                        )}
                    </div>
                    <div
                ...
                <div className="min-h-[60px]">
                    {activeBriefTab === 'brief' ? (
                        ...
                        >
                            <div
                                className="w-10 h-10 rounded-full bg-indigo-500/10 flex items-center justify-center shrink-0 border border-indigo-500/30">
                            ...
                            <div>
                                <p className="text-[#f0f6fc] text-base leading-relaxed font-medium">
                                    {tasks.length === 0 && completedTasks.length === 0 ? (
                                        ...
                                        <span>Your workload requires attention. You have <strong
                                            className="text-indigo-300">{tasksAtRisk} task(s)</strong> at risk of missing deadlines. I strongly suggest prioritizing <strong
                                            className="text-indigo-400">{topTask?.title}</strong> today.</span>
                                    ) : tasks.length > 0 ? (
                                        <span>Your schedule looks perfectly balanced today. You have <strong
                                            className="text-indigo-300">{tasks.length} pending task(s)</strong> with no immediate risks detected. Your productivity score is currently at {productivityScore}%. Keep up the momentum!</span>
                                    ) : (
                        ...
                        decisions.length > 0 && (
                            ...
                            >
                                <div
                                    className="w-10 h-10 rounded-full bg-emerald-500/10 flex items-center justify-center shrink-0 border border-emerald-500/30">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4 flex-grow">
                        ...
                        <div className="relative z-10 flex flex-col h-full">
                            <div className="flex items-center justify-between mb-6">
                                <div className="flex items-center gap-2">
                                    <span
                                        className="px-2 py-1 bg-indigo-500/20 text-indigo-400 text-[10px] font-bold rounded uppercase tracking-wider">Active Execution</span>
                                </div>
                                {tasks.length > 0 && (
                                    <Button onClick={() => setShowSelectionModal(true)}
                                            disabled={isGenerating || isJobActive || loadingTasks} size="sm"
                                            className="bg-white text-indigo-900 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-indigo-50 transition-colors shadow-lg">
                            ...
                            <h2 className="text-3xl font-light text-white mb-2 leading-tight">Today's
                                Execution <br/><span className="font-semibold italic text-indigo-300">Plan</span></h2>
                            <p className="text-slate-400 max-w-md mb-8">Your AI-optimized schedule based on deadlines
                                and priorities.</p>
                            ...
                            <div className="flex-grow">
                                {!plan && !isGenerating && !isJobActive && (
                                    tasks.length === 0 ? (
                                        <div
                                            className="text-center py-12 px-4 bg-indigo-500/5 border border-indigo-500/10 rounded-2xl">
                    <span className="inline-block p-3 rounded-full bg-indigo-500/10 text-indigo-400 mb-3">
                      <Sparkles className="w-6 h-6"/>
                    </span>
                                            <h4 className="text-sm font-semibold text-white">Your Dashboard is
                                                Clear</h4>
                                            <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">You don't have
                                                any pending tasks right now. Go to the Mission Board to add tasks and
                                                define your quests!</p>
                                        </div>
                                    ) : (
                                    ...
                                    <div className="space-y-3">
                                        {(() => {
                                            ...
                                            if (tasks.length === 0) {
                                                return (
                                                    <div
                                                        className="text-center py-12 px-4 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl">
                          <span
                              className="inline-block p-3 rounded-full bg-emerald-500/10 text-emerald-400 mb-3 animate-bounce">
                            <Sparkles className="w-6 h-6"/>
                          </span>
                                                        <h4 className="text-sm font-semibold text-white">Execution Plan
                                                            Fully Completed!</h4>
                                                        <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">Fantastic
                                                            job, Pilot! All scheduled tasks have been finished. Your
                                                            daily dashboard is completely clear.</p>
                                                    </div>
                                                );
                                            }
                                            ...
                                            if (visibleSessions.length === 0) {
                                                return (
                                                    <div className="text-center py-12 text-slate-500">
                                                        <p className="text-sm">No scheduled sessions for your pending
                                                            tasks. Click Reschedule to build a new timeline.</p>
                                                    </div>
                                                );
                                            }
                        ...
                        {loadingTasks ? (
                            ...
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <p className="text-[10px] text-indigo-200 uppercase font-bold tracking-widest">Tasks
                                        Remaining</p>
                                </div>
                                <div className="space-y-3">
                                    {tasks.slice(0, 4).map(t => (
                                        <div key={t.id}
                                             className="flex items-center justify-between p-3 bg-white/10 rounded-2xl rounded-tr-none border border-indigo-400/30">
                                    ...
                                    {goals.filter(g => !g.completed).slice(0, 3).map(g => (
                                        <div key={g.id}
                                            ...
                                            <div className="flex items-center justify-between gap-2">
                                                ...
                                                <span
                                                    className={`text-[8px] px-1.5 py-0.5 rounded font-bold uppercase shrink-0 ${g.type === 'habit' ? 'bg-orange-500/10 text-orange-400 border border-orange-500/20' : 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20'}`}>
                          {g.type}
                        </span>
                                            </div>
                                            {g.type === 'quest' ? (
                                                <div className="flex items-center gap-3 w-full justify-between mt-1">
                                                    <span
                                                        className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">Progress</span>
                                                    <CircularProgress progress={g.progress} size={28} strokeWidth={3.5}
                                                                      color="stroke-cyan-400">
                                                        <span
                                                            className="text-[8px] font-mono font-bold text-cyan-400">{g.progress}%</span>
                                                    </CircularProgress>
                                                </div>
                                            ) : (
                                                <div className="flex items-center gap-2 mt-1">
                                                    <motion.div
                                                        ...
                                                        <span
                                                            className="text-xs font-bold font-mono">{g.streak || 0}</span>
                                                    </motion.div>
                                                    <span
                                                        className="text-[9px] text-slate-500 font-medium uppercase tracking-widest">Day Streak</span>
                                                </div>
                                            )}
                                        </div>
                                    ))}
                ...
                >
                    ...
                    <div className="space-y-4">
                        {decisions.length === 0 && (
                            <div className="text-[10px] text-slate-500 italic">No decisions yet. Generate a schedule to
            ...
            >
                ...
                >
                    <div className="relative flex items-center justify-center">
                        <span
                            className="absolute inline-flex h-2.5 w-2.5 rounded-full bg-cyan-400 opacity-75 animate-ping"></span>
